...
from helper.tcp_stream import TCPStream
from helper.udp_stream import UDPStream
import argparse
from helper.entropy import kolmogorov, shannon
import warnings
import numpy as np
attrs = ['src','sport','dst',
...
def get_flows(flows, protocol):
    ...
    for flow in flows.values():
        #flow_average_length = round(flow.avrg_len(), 2)
        #flow_averag_payload = round(flow.avrg_payload_len(), 2)
        #load = str(flow_average_length) + ':' + str(flow_averag_payload)
        each_packet = []
